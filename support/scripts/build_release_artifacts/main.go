...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/andskur/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/andskur/go/support/app.version=%s", version)
